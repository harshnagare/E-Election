...
while True:
   ...
   # Resize the frame to match the target region size
    resized_frame = cv2.resize(frame, (500, 450))  # Resize to (width, height)

    # Assign the resized frame to the target region
    imgBackground[300:300 + 450, 150:150 + 500] = resized_frame
    ...
    if output is not None:
        ...
        if k == ord('3'):
            ...
            if exist:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
                    attendance = [output[0], "SHIVSENA", date, timestamp]
                    writer.writerow(attendance)
            else:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
        ...
        if k == ord('4'):
            ...
            if exist:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
                    attendance = [output[0], "MNS", date, timestamp]
                    writer.writerow(attendance)
            else:
                with open("Votes.csv", "a") as csvfile:
                    writer = csv.writer(csvfile)
